...
function getCityOverlap(currentWindows = [], targetWindows = []) {
  for (const currentWindow of currentWindows) {
    ...
    for (const targetWindow of targetWindows) {
      ...
      if (
        currentWindow.city_area.trim().toLowerCase() !== targetWindow.city_area.trim().toLowerCase()
      ) {
        continue;
      }
...
export function buildCompatibilityChips(currentProfile, targetProfile, max = 4) {
  ...
  if (currentProfile.travel_pace && targetProfile.travel_pace) {
    if (currentProfile.travel_pace === targetProfile.travel_pace) chips.push('Same travel pace');
  }

  if (currentProfile.profile_type && targetProfile.profile_type) {
    if (currentProfile.profile_type === targetProfile.profile_type) chips.push('Similar travel setup');
  }
  ...
  if (currentProfile.now_in_city && targetProfile.now_in_city) {
    if (
      currentProfile.now_in_city.trim().toLowerCase() === targetProfile.now_in_city.trim().toLowerCase()
    ) {
